  ...
  // UPLOAD DOCUMENT FOR VISIT
    public DocumentSummaryResponse uploadForVisit(DocumentUploadRequest request) throws IOException {
        ...
        Visit visit = visitRepository.findById(request.getVisitId())
                .orElseThrow(() -> new IllegalArgumentException("There is no such data available on this id " + request.getVisitId()));
        ...
        Document saved = documentRepository.save(Mapper.toDocumentEntity(request, visit, filePath, file.getContentType()));
    ...
    public List<DocumentSummaryResponse> getDocumentsForVisit(Long visitId) {
        Visit visit = visitRepository.findById(visitId)
                .orElseThrow(() -> new IllegalArgumentException("Invalid visitId: " + visitId));

        List<Document> docs = documentRepository.findByVisitOrderByReportDateDesc(visit);

        return docs.stream()
                .map(Mapper::toDocumentSummaryResponse)
