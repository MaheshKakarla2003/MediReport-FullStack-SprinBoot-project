...
public class VisitServiceImpl implements VisitService {
    ...
    public VisitSummaryResponse createVisit(VisitCreateRequest request) {
        Patient patient = patientRepository.findById(request.getPatientId())
                .orElseThrow(() -> new IllegalArgumentException("No such Patient available with this  Id " + request.getPatientId()));

        Hospital hospital = hospitalRepository.findById(request.getHospitalId())
                .orElseThrow(() -> new IllegalArgumentException("No such Hospital available with this  Id " + request.getHospitalId()));
    ...
    public List<VisitSummaryResponse> getVisitsForPatientAndHospital(Long patientId, Long hospitalId) {
        Patient patient = patientRepository.findById(patientId)
                .orElseThrow(() -> new IllegalArgumentException("No such Patient available with this  Id " + patientId));

        Hospital hospital = hospitalRepository.findById(hospitalId)
                .orElseThrow(() -> new IllegalArgumentException("No such Hospital available with this  Id " + hospitalId));
    ...
    public VisitSummaryResponse getVisitById(Long visitId) {
        Visit visit = visitRepository.findById(visitId)
                .orElseThrow(() -> new IllegalArgumentException("No such Visit Data available with this  Id " + visitId));
	...
	public String deleteVisit(long id) {
		 ...
		 Visit visit = visitRepository.findById(id)
	                .orElseThrow(() -> new IllegalArgumentException("No such Visit Data available with this  Id " + id));
